...
class OpenAI:
    ...
    def generate(self, text, img, temperature=0, prefix=""):
        """
        Sends a chat completion request to the OpenAI API using the specified messages and parameters.
...
class OLLAMA:
    ...
    def generate(self, prompt, images, itemperature=0):
        ...
        payload = {"model": self.model_name, "prompt": prompt, "stream": False, "images": images}

        headers = {"Content-Type": "application/json"}

        response = requests.post(
            self.llama_serve_generate, data=json.dumps(payload), headers=headers
        )
...
def test_vision():
    ...
    if MODEL_TYPE == "OpenAI":
        llm = OpenAI()
        capture_path = screen_capture_pyautogui(save_dir)
        response = llm.generate(prompt, encode_image(capture_path))
